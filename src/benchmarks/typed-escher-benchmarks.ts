...
const loadBenchmarksFromJsonDir = (benchmarkDir: string): readonly TypedEscherBenchmarkCase[] => {
  ...
  return files.map((file) => {
    ...
    const declaredComponents = [...new Set((spec.components ?? []).map((component) => component.name))].sort();
    const declaredSet = new Set(declaredComponents);
    const autoGeneratedComponents = [...job.env.keys()].filter((name) => !declaredSet.has(name)).sort();
    return {
      ...
      env: job.env,
      ...(job.recursiveInvariantArgIndices.length > 0
        ? {
            synthConfigOverride: {
              recursiveInvariantArgIndices: job.recursiveInvariantArgIndices,
            },
          }
        : {}),
      declaredComponents,
